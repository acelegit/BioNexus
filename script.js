...
var I18N = {
  ro: {
    ...
    "load.muscleText": "Se încarcă sistemul muscular…",
  ...
  en: {
    ...
    "load.muscleText": "Loading muscular system…",
...
(function bxEnterSubmit() {
  var MAP = [
    ["#authBodyLogin", "#loginSubmitBtn"],
    ["#authBodyRegister", "#regSubmitBtn"],
    ["#forgotPassModal", "#forgotSubmitBtn"],
    ["#resetPassModal", "#resetSubmitBtn"],
    ["#adminAuth", "[onclick*='adminLogin']"],
    ["#reportModal", "#reportSend"],
    ["#profileNameEditRow", "[onclick*='saveName']"],
    [".online-join-row", "[onclick*='onlineJoinRoom']"],
  ];
  document.addEventListener("keydown", function (e) {
    ...
    if (["checkbox", "radio", "file", "range", "hidden", "button", "submit", "reset"].indexOf(type) >= 0) return;
    if ((t.id && /search/i.test(t.id)) || t.closest(".search-box")) return;
    if (t.getAttribute("tabindex") === "-1" || t.getAttribute("aria-hidden") === "true") return;
    function fire(btn) {
    ...
    if (/^settings(Cur|New|Conf)Pass$/.test(t.id)) {
      if (fire(document.querySelector("[onclick*='changePassword']"))) return;
    }
    for (var i = 0; i < MAP.length; i++) {
      var scope = t.closest(MAP[i][0]);
